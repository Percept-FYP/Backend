...
def post(request):
    ...
    if serializer.is_valid():
        ...
        abs_list=list(set(list_all) - set(present_list)) + list(set(present_list) - set(list_all))
        ...
        for usn in present_list:
            attend = attendance.objects.create(usn=usn, attend="present", class_id=class_na)
        for usn in abs_list:
            attend = attendance.objects.create(usn=usn, attend="absent", class_id=class_na)
            attendances = attendance.objects.filter(class_id=class_na).order_by("usn")
        workbook1 = openpyxl.load_workbook('D:/FYP/main/backend/_backend/DB.xlsx') 
